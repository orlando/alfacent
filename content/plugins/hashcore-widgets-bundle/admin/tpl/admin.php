<div class="wrap" id="sow-widgets-page">
	<div class="page-banner">
		...
		<h1><?php _e('HashCore Widgets Bundle', 'hashcore-widgets-bundle') ?></h1>

		<div id="sow-widget-search">
			<input type="search" placeholder="<?php esc_attr_e('Filter Widgets', 'hashcore-widgets-bundle') ?>" />
		</div>
	</div>

	<ul class="page-nav">
		<li class="active"><a href="#all"><?php _e('All', 'hashcore-widgets-bundle') ?></a></li>
		<li><a href="#enabled"><?php _e('Enabled', 'hashcore-widgets-bundle') ?></a></li>
		<li><a href="#disabled"><?php _e('Disabled', 'hashcore-widgets-bundle') ?></a></li>
	</ul>
		...
		<?php foreach( $widgets as $id => $widget ): ?>
			<div class="so-widget-wrap">
				<div class="so-widget so-widget-is-<?php echo $widget['Active'] ? 'active' : 'inactive' ?>" data-id="<?php echo esc_attr( $widget['ID'] ) ?>">
					...
					$banner = apply_filters('hashcore_widgets_widget_banner', $banner, $widget);
					?>
					...
					<div class="so-widget-text">
						...
						<div class="so-widget-active-indicator"><?php _e('Active', 'hashcore-widgets-bundle') ?></div>
						...
						<div class="so-widget-toggle-active">
							<button class="button-secondary so-widget-activate" data-status="1"><?php esc_html_e( 'Activate', 'hashcore-widgets-bundle' ) ?></button>
							<button class="button-secondary so-widget-deactivate" data-status="0"><?php esc_html_e( 'Deactivate', 'hashcore-widgets-bundle' ) ?></button>
						</div>
	...
	<div class="developers-link">
		<?php _e('Developers - create your own widgets for the Widgets Bundle.', 'hashcore-widgets-bundle') ?>
		<a href="https:///docs/widgets-bundle/" target="_blank"><?php _e('Read More', 'hashcore-widgets-bundle') ?></a>.
	</div>
