(function($){
    ...
    $(document).on( 'sowsetupform', '.hashcore-widget-form[data-class="HashCore_Widget_SocialMediaButtons_Widget"]', function() {
        var $socialMediaForm = $(this);

        var setNetworkDefaults = function($selectNetworkInput) {
            window.sowForms.fetchWidgetVariable('networks', 'HashCore_Widget_SocialMediaButtons_Widget',
                function(networks) {
                    var selectedNetwork = networks[$selectNetworkInput.find(':selected').val()];
                    var $closestForm = $selectNetworkInput.closest('.hashcore-widget-field-repeater-item-form');
