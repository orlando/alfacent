...
<div class="ow-pt-columns-atom">
				...
				<?php foreach($column['features'] as $i => $feature) : ?>
					<div class="ow-pt-feature ow-pt-feature-<?php echo $i % 2 == 0 ? 'even' : 'odd' ?>">
						...
						if( !empty($feature['icon_new']) ) {
							...
							echo hashcore_widget_get_icon($feature['icon_new'], $icon_styles);
						}
						?>
	...
	<?php
	global $hashcore_price_table_icons;
	if( empty($hashcore_price_table_icons) ) $hashcore_price_table_icons = array();
	foreach($instance['columns'] as $i => $column){
		foreach($column['features'] as $feature) {
			if(!empty($feature['icon']) && empty($hashcore_price_table_icons[$feature['icon']])) {
				$hashcore_price_table_icons[$feature['icon']] = true;
