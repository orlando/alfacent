...
Author:
Author URI: https://
Plugin URI: https:///widgets-bundle/
License: GPL3
...
class HashCore_Widgets_Bundle {
	...
	function __construct(){
		...
		add_action( 'hashcore_widgets_version_update', array( $this, 'handle_update' ), 10, 2 );
		...
		add_filter( 'hashcore_panels_data', array($this, 'load_missing_widgets') );
		add_filter( 'hashcore_panels_prebuilt_layout', array($this, 'load_missing_widgets') );
		add_filter( 'hashcore_panels_widget_object', array($this, 'load_missing_widget'), 10, 2 );
	 ...
	 *
	 * @return HashCore_Widgets_Bundle
	 */
	static function single() {
		static $single;

		if( empty($single) ) {
			$single = new HashCore_Widgets_Bundle();
		}
	...
	function set_plugin_textdomain(){
		load_plugin_textdomain('hashcore-widgets-bundle', false, dirname( plugin_basename( __FILE__ ) ). '/languages/');
	}
	...
	function plugin_version_check(){
		...
		$active_version = get_option( 'hashcore_widget_bundle_version' );

		$is_new = empty($active_version) || version_compare( $active_version, SOW_BUNDLE_VERSION, '<' );
		$is_new = apply_filters( 'hashcore_widgets_is_new_version', $is_new );

		if( $is_new ) {

			update_option( 'hashcore_widget_bundle_version', SOW_BUNDLE_VERSION );
			// If this is a new version, then trigger an action to let widgets handle the updates.
			do_action( 'hashcore_widgets_version_update', SOW_BUNDLE_VERSION, $active_version );
	 ...
	 *
	 * @action hashcore_widgets_version_update
	 */
	 ...
	 * Can also be called directly on the `HashCore_Widgets_Bundle` singleton class.
	 *
	...
	function clear_widget_cache() {
		// Remove all cached CSS for HashCore Widgets
	...
	function check_for_new_widgets() {
		...
		$old_widgets = get_option( 'hashcore_widgets_old_widgets' );
		...
		if ( empty( $old_widgets ) ) {
			update_option( 'hashcore_widgets_old_widgets', implode( ',', $widgets ) );
			return;
		}
		$old_widgets = explode( ',', $old_widgets );
		$new_widgets = array_diff( $widgets, $old_widgets );
		if ( ! empty( $new_widgets ) ) {
			update_option( 'hashcore_widgets_new_widgets', $new_widgets );
			update_option( 'hashcore_widgets_old_widgets', implode( ',', $widgets ) );
		}
	}

	function display_admin_notices() {
		$new_widgets = get_option( 'hashcore_widgets_new_widgets' );
		if ( empty( $new_widgets ) ) {
			return;
		}
		?>
		<div class="updated">
			<p><?php echo __( 'New widgets available in the ') . '<a href="' . admin_url('plugins.php?page=so-widgets-plugins') . '">' . __('HashCore Widgets Bundle', 'hashcore-widgets-bundle' ) . '</a>!'; ?></p>
			<?php
			...
			foreach ( $new_widgets as $widget_file_path ) {
				...
				$widget = get_file_data( $widget_file_path, $default_headers, 'hashcore-widget' );
				$name = empty( $widget['Name'] ) ? $match[1] : $widget['Name'];
				$description = empty( $widget['Description'] ) ? __( 'A new widget!', 'hashcore-widgets-bundle' ) : $widget['Description'];
				?>
		...
		<?php
		update_option( 'hashcore_widgets_new_widgets', array() );
	}
	...
	function get_widget_folders(){
		if( empty($this->widget_folders) ) {
			...
			$this->widget_folders = apply_filters('hashcore_widgets_widget_folders', array(
	...
	function get_active_widgets( $filter = true ){
		...
		$active_widgets = wp_cache_get( 'active_widgets', 'hashcore_widgets' );

		if( empty($active_widgets) ) {
			$active_widgets = get_option( 'hashcore_widgets_active', array() );
			$active_widgets = wp_parse_args( $active_widgets, apply_filters( 'hashcore_widgets_default_active', self::$default_active_widgets ) );
			...
			if ( $filter ) {
				$active_widgets = apply_filters( 'hashcore_widgets_active_widgets', $active_widgets );
			}

			wp_cache_add( 'active_widgets', $active_widgets, 'hashcore_widgets' );
		}
	...
	function admin_enqueue_scripts($prefix) {
		...
		wp_enqueue_style( 'hashcore-widgets-manage-admin', plugin_dir_url( __FILE__ ) . 'admin/admin.css', array(), SOW_BUNDLE_VERSION );
		wp_enqueue_script( 'hashcore-widgets-trianglify', plugin_dir_url( __FILE__ ) . 'admin/trianglify' . SOW_BUNDLE_JS_SUFFIX . '.js', array(), SOW_BUNDLE_VERSION );
		wp_enqueue_script( 'hashcore-widgets-manage-admin', plugin_dir_url( __FILE__ ) . 'admin/admin' . SOW_BUNDLE_JS_SUFFIX . '.js', array(), SOW_BUNDLE_VERSION );

		wp_localize_script( 'hashcore-widgets-manage-admin', 'soWidgetsAdmin', array(
	...
	function admin_activate_widget() {
		if(
			...
			&& isset($_GET['_wpnonce'])
			&& wp_verify_nonce($_GET['_wpnonce'], 'hashcore_widget_action')
		) {
	...
	function admin_ajax_manage_handler(){
		...
		if( ! current_user_can( apply_filters( 'hashcore_widgets_admin_menu_capability', 'manage_options' ) ) ) exit();
	...
	function admin_menu_init(){
		add_plugins_page(
			__('HashCore Widgets', 'hashcore-widgets-bundle'),
			__('HashCore Widgets', 'hashcore-widgets-bundle'),
			apply_filters('hashcore_widgets_admin_menu_capability', 'manage_options'),
			'so-widgets-plugins',
	...
	function admin_page(){
		...
		$bundle = HashCore_Widgets_Bundle::single();
		...
		if(
			...
			<div class="updated">
				...
				<?php
				printf(
					__('%s was %s', 'hashcore-widgets-bundle'),
					$widgets[ $_GET['widget'].'/'.$_GET['widget'].'.php' ]['Name'],
					$_GET['widget_action'] == 'activate' ? __('Activated', 'hashcore-widgets-bundle') : __('Deactivated', 'hashcore-widgets-bundle')
				)
				?>
				</p>
			</div>
	...
	function activate_widget( $widget_id, $include = true ){
		...
		wp_cache_delete( 'hashcore_widgets_active', 'options' );
		$active_widgets = $this->get_active_widgets();
		$active_widgets[$widget_id] = true;
		update_option( 'hashcore_widgets_active', $active_widgets );
		wp_cache_delete( 'active_widgets', 'hashcore_widgets' );
		...
		foreach( $this->widget_folders as $folder ) {
			...
			if( has_action('widgets_init') ) {
				HashCore_Widgets_Widget_Manager::single()->widgets_init();
			}
		}
	...
	function get_widgets_list(){
		...
		foreach( $folders as $folder ) {
			...
			foreach($files as $file) {
				$widget = get_file_data( $file, $default_headers, 'hashcore-widget' );
	...
	function load_missing_widgets($data){
		...
		foreach($data['widgets'] as $widget) {
			...
			if( preg_match('/HashCore_Widget_([A-Za-z]+)_Widget/', $class, $matches) ) {
				$name = $matches[1];
			...
			if ( ! empty( $widgets ) && $sidebar !== "wp_inactive_widgets") {
				foreach ( $widgets as $i => $id ) {
					if ( ! empty( $wp_registered_widgets[$id] ) ) {
						...
						if ( !empty($widget) && is_object($widget) && is_subclass_of($widget, 'HashCore_Widget') && is_active_widget( false, false, $widget->id_base ) ) {
...
HashCore_Widgets_Bundle::single();

// Initialize the Meta Box Manager
global $sow_meta_box_manager;
$sow_meta_box_manager = HashCore_Widget_Meta_Box_Manager::single();
