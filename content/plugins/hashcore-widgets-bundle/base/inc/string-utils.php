 ...
 */
function hashcore_widgets_underscores_to_camel_case( $array ) {
	$transformed = array();
	if ( !empty( $array ) ) {
		foreach ( $array as $key => $val ) {
			$jsKey = preg_replace_callback( '/_(.?)/', 'hashcore_widgets_match_to_upper', $key );
 ...
 * @return string
 */
function hashcore_widgets_match_to_upper( $matches ) {
