...
(function( $ ) {
    var setup = function(widgetForm) {
        if(typeof tinyMCEPreInit !== 'undefined') {
            ...
            $(widgetForm).find('> .hashcore-widget-field-type-tinymce > .hashcore-widget-tinymce-container').each(function (index, element) {
                ...
                qtSettings = $.extend({}, tinyMCEPreInit.qtInit['hashcore-widget-input-tinymce-field'], qtSettings, {id: id});
                ...
                $(this).on( 'click', function(event) {
                    ...
                    if ( $target.hasClass( 'wp-switch-editor' ) ) {
                        ...
                        $(this).find('+ .hashcore-widget-tinymce-selected-editor').val(mode);
                    }
                });
            });
            QTags._buttonsInit();
    ...
    $(document).on( 'sowsetupform', function(e) {
        ...
        if($f.is('.hashcore-widget-field-repeater-item-form')){
            if($f.is(':visible')) {
                setup( $f );
            }
            else {
    ...
    $(document).on('sortstop', function (event, ui) {
        if(ui.item.is('.hashcore-widget-field-repeater-item')) {
            ui.item.find('> .hashcore-widget-field-repeater-item-form').each(function(){
                setup( $(this) );
            });
        }
        else {
            setup(ui.item.find('.hashcore-widget-form'));
        }
    });
