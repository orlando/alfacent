 ...
 *
 * Class HashCore_Widget_Field_Media
 */
class HashCore_Widget_Field_Media extends HashCore_Widget_Field_Base {
	/**
	...
	protected function get_default_options() {
		return array(
			'choose' => __( 'Choose Media', 'hashcore-widgets-bundle' ),
			'update' => __( 'Set Media', 'hashcore-widgets-bundle' ),
			'library' => 'image'
		);
	}
	...
	protected function render_field( $value, $instance ) {
		if( version_compare( get_bloginfo('version'), '3.5', '<' ) ){
			printf( __('You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'hashcore-widgets-bundle'), admin_url('update-core.php') );
			return;
		}
		...
		<a href="#" class="media-remove-button <?php if( empty( $value ) ) echo 'remove-hide'; ?>"><?php esc_html_e( 'Remove', 'hashcore-widgets-bundle' ) ?></a>

		<input type="hidden" value="<?php echo esc_attr( is_array( $value ) ? '-1' : $value ) ?>" name="<?php echo esc_attr( $this->element_name ) ?>" class="hashcore-widget-input" />
	...
	protected function render_after_field( $value, $instance ) {
		if( !empty( $this->fallback ) ) {
			...
			<input type="text" value="<?php echo esc_url( $fallback_url ) ?>"
			       placeholder="<?php esc_attr_e( 'External URL', 'hashcore-widgets-bundle' ) ?>"
			       name="<?php echo esc_attr( $this->for_widget->so_get_field_name( $this->base_name . '_fallback', $this->parent_container ) ) ?>"
			       class="media-fallback-external hashcore-widget-input" />
			<div class="clear"></div>
			<?php
		}
		else {
