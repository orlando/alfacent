...
/**
 * The base class for all HashCore_Widget form fields.
 *
 * Class HashCore_Widget_Field
 */

abstract class HashCore_Widget_Field_Base {
	 ...
	 * the pattern `'hashcore_widgets_sanitize_field_' . $sanitize`, in case the sanitization is defined elsewhere.
	 *
	 * @access protected
	 * @var string
	 */
	...
	/* See https:///docs/widgets-bundle/form-building/state-emitters/ for more detail on the topic of   */
	 ...
	 *
	 * @param HashCore_Widget $for_widget
	...
	public function __construct( $base_name, $element_id, $element_name, $field_options, $for_widget = null, $parent_container = array() ) {
		if( isset( $field_options['type'] ) ) {
			...
			throw new InvalidArgumentException( 'HashCore_Widget_Field_Base::__construct: $field_options must contain a \'type\' field.' );
		}
	...
	protected function get_label_classes( $value, $instance ) {
		return array( 'hashcore-widget-field-label' );
	}
	...
	protected function get_description_classes() {
		return array( 'hashcore-widget-field-description' );
	}

	/**
	 * This function is called by the containing HashCore_Widget when rendering it's form.
	 *
	...
	public function render( $value, $instance = array() ) {
		...
		$wrapper_attributes = array(
			'class' => array(
				'hashcore-widget-field',
				'hashcore-widget-field-type-' . $this->type,
				'hashcore-widget-field-' . $this->base_name
			)
		);

		if( !empty( $this->optional ) ) $wrapper_attributes['class'][] = 'hashcore-widget-field-is-optional';
	...
	protected function render_field_label( $value, $instance ) {
		...
		if( !empty( $this->optional ) ) {
			echo '<span class="field-optional">(' . __('Optional', 'hashcore-widgets-bundle') . ')</span>';
		}
		?>
		</label>
		<?php
		...
		if( isset( $this->sanitize ) ) {
			...
			switch( $this->sanitize ) {
				...
				default:
					...
					$value = apply_filters( 'hashcore_widgets_sanitize_field_' . $this->sanitize, $value );
					break;
			}
		}
