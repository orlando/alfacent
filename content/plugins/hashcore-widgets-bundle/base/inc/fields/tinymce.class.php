...
class HashCore_Widget_Field_TinyMCE extends HashCore_Widget_Field_Text_Input_Base {
	/**
	...
	function __call( $name, $arguments ) {
		if ( preg_match( '/mce_buttons(?:_[1-4])?|quicktags_settings/', $name ) && !empty( $this->button_filters[$name] ) ) {
			...
			if ( !empty( $filter[0] ) && is_a( $filter[0], 'HashCore_Widget' ) ) {
				$widget = $filter[0];
	...
	protected function render_field( $value, $instance ) {
		...
		$settings = array(
			...
			'editor_class' => 'hashcore-widget-input',
			'tinymce' => array(
		...
		?>
		<div class="hashcore-widget-tinymce-container"
		...
		<input type="hidden"
			   ...
			   class="hashcore-widget-input hashcore-widget-tinymce-selected-editor"
