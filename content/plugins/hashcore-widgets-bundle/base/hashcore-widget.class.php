...
/**
 * Class HashCore_Widget
 *
 * @author HashCore <support@>
 */
abstract class HashCore_Widget extends WP_Widget {
	 ...
	 * @param array $form_options Optional An array describing the form fields used to configure HashCore widgets.
	...
	function __construct($id, $name, $widget_options = array(), $control_options = array(), $form_options = array(), $base_folder = false) {
		...
		do_action('hashcore_widgets_initialize_widget_' . $this->id_base, $this);
	}
	 ...
	 *
	 * @param bool|HashCore_Widget $parent
	 *
	 * @return mixed
	 */
	function form_options( $parent = false ) {
		...
		$form_options = apply_filters( 'hashcore_widgets_form_options', $form_options, $this );
		$form_options = apply_filters( 'hashcore_widgets_form_options_' . $this->id_base, $form_options, $this );
		return $form_options;
	}
	...
	public function widget( $args, $instance ) {
		...
		// Filter the instance
		$instance = apply_filters( 'hashcore_widgets_instance', $instance, $this );
		$instance = apply_filters( 'hashcore_widgets_instance_' . $this->id_base, $instance, $this );
		...
		$template_vars = apply_filters( 'hashcore_widgets_template_variables_' . $this->id_base, $template_vars, $instance, $args, $this );
		...
		$template_file = hashcore_widget_get_plugin_dir_path( $this->id_base ) . $this->get_template_dir( $instance ) . '/' . $this->get_template_name( $instance ) . '.php';
		$template_file = apply_filters('hashcore_widgets_template_file_' . $this->id_base, $template_file, $instance, $this );
		...
		$template_html = apply_filters( 'hashcore_widgets_template', $template_html, get_class($this), $instance, $this );
		$template_html = apply_filters( 'hashcore_widgets_template_html_' . $this->id_base, $template_html, $instance, $this );
		echo $template_html;
		echo '</div>';
		...
		if( !empty($style) ) {
			...
			if ( ! in_array( $css_name, $this->generated_css ) || $in_preview ) {
				if( $in_preview ) {
					...
					if( file_exists( $upload_dir['basedir'] . '/hashcore-widgets/' . $css_name .'.css' ) ) {
						if ( ! wp_style_is( $css_name ) ) {
							wp_enqueue_style(
								$css_name,
								set_url_scheme($upload_dir['baseurl'] . '/hashcore-widgets/' . $css_name .'.css')
							);
						}
					}
					else {
						...
						$css = wp_cache_get( $css_name, 'hashcore_widgets' );
						if ( empty( $css ) ) {
							$css = $this->get_instance_css( $instance );
						}
						hashcore_widget_add_inline_css( $css );
					}
				}
	...
	public function form( $instance ) {
		...
		$instance = apply_filters('hashcore_widgets_form_instance_' . $this->id_base, $instance, $this);

		$form_id = 'hashcore_widget_form_'.md5( uniqid( rand(), true ) );
		...
		?>
		<div class="hashcore-widget-form hashcore-widget-form-main hashcore-widget-form-main-<?php echo esc_attr($class_name) ?>" id="<?php echo $form_id ?>" data-class="<?php echo get_class($this) ?>" style="display: none">
			<?php
			/* @var $field_factory HashCore_Widget_Field_Factory */
			$field_factory = HashCore_Widget_Field_Factory::getInstance();
			$fields_javascript_variables = array();
			foreach( $this->form_options() as $field_name => $field_options ) {
				/* @var $field HashCore_Widget_Field_Base */
			...
			?>
			<input type="hidden" name="<?php echo $this->get_field_name('_sow_form_id') ?>" value="<?php echo esc_attr( $instance['_sow_form_id'] ) ?>" class="hashcore-widgets-form-id" />
		</div>
		<div class="hashcore-widget-form-no-styles">
			<p><strong><?php _e('This widget has scripts and styles that need to be loaded before you can use it. Please save and reload your current page.', 'hashcore-widgets-bundle') ?></strong></p>
			<p><strong><?php _e('You will only need to do this once.', 'hashcore-widgets-bundle') ?></strong></p>
		</div>

		<?php if( $this->widget_options['has_preview'] && ! $this->is_customize_preview() ) : ?>
			<div class="hashcore-widget-preview" style="display: none">
				<a href="#" class="hashcore-widget-preview-button button-secondary"><?php _e('Preview', 'hashcore-widgets-bundle') ?></a>
			</div>
		<?php endif; ?>

		<?php if( !empty( $this->widget_options['help'] ) ) : ?>
			<a href="<?php echo sow_esc_url($this->widget_options['help']) ?>" class="hashcore-widget-help-link hashcore-panels-help-link" target="_blank"><?php _e('Help', 'hashcore-widgets-bundle') ?></a>
		<?php endif; ?>
	...
	function enqueue_scripts(){
		...
		if( !wp_script_is('hashcore-widget-admin') ) {
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_style( 'hashcore-widget-admin', plugin_dir_url(SOW_BUNDLE_BASE_FILE).'base/css/admin.css', array( 'media-views' ), SOW_BUNDLE_VERSION );


			wp_enqueue_script( 'wp-color-picker' );
			wp_enqueue_media();
			wp_enqueue_script( 'hashcore-widget-admin', plugin_dir_url(SOW_BUNDLE_BASE_FILE).'base/js/admin' . SOW_BUNDLE_JS_SUFFIX . '.js', array( 'jquery', 'jquery-ui-sortable', 'jquery-ui-slider' ), SOW_BUNDLE_VERSION, true );

			wp_localize_script( 'hashcore-widget-admin', 'soWidgets', array(
				'ajaxurl' => wp_nonce_url( admin_url('admin-ajax.php'), 'widgets_action', '_widgets_nonce' ),
				'sure' => __('Are you sure?', 'hashcore-widgets-bundle')
			) );
		...
		if( $this->using_posts_selector() ) {
			hashcore_widget_post_selector_enqueue_admin_scripts();
		}

		// This lets the widget enqueue any specific admin scripts
		$this->enqueue_admin_scripts();
		do_action( 'hashcore_widgets_enqueue_admin_scripts_' . $this->id_base, $this );
	}
	...
	function footer_admin_templates(){
		...
		<script type="text/template" id="so-widgets-bundle-tpl-preview-dialog">
			<div class="hashcore-widget-preview-dialog">
				<div class="hashcore-widgets-preview-modal-overlay"></div>

				<div class="so-widget-toolbar">
					<h3><?php _e('Widget Preview', 'hashcore-widgets-bundle') ?></h3>
					<div class="close"><span class="dashicons dashicons-arrow-left-alt2"></span></div>
				</div>

				<div class="so-widget-iframe">
					<iframe name="hashcore-widget-preview-iframe" id="hashcore-widget-preview-iframe" style="visibility: hidden"></iframe>
				</div>

				<form target="hashcore-widget-preview-iframe" action="<?php echo wp_nonce_url( admin_url('admin-ajax.php'), 'widgets_action', '_widgets_nonce' ) ?>" method="post">
		...
		do_action('hashcore_widgets_footer_admin_templates');
	}
	...
	public function update( $new_instance, $old_instance ) {
		if( !class_exists('HashCore_Widgets_Color_Object') ) require plugin_dir_path( __FILE__ ).'inc/color.php';
		...
		if( ! empty( $form_options ) ) {
			...
			$new_instance = apply_filters( 'hashcore_widgets_sanitize_instance', $new_instance, $form_options, $this );
			$new_instance = apply_filters( 'hashcore_widgets_sanitize_instance_' . $this->id_base, $new_instance, $form_options, $this );
		}
	...
	private function save_css( $instance ){
		...
		if( !empty($css) ) {
			...
			if ( WP_Filesystem() ) {
				...
				if ( ! $wp_filesystem->is_dir( $upload_dir['basedir'] . '/hashcore-widgets/' ) ) {
					$wp_filesystem->mkdir( $upload_dir['basedir'] . '/hashcore-widgets/' );
				}

				$wp_filesystem->delete( $upload_dir['basedir'] . '/hashcore-widgets/' . $name );
				$wp_filesystem->put_contents(
					$upload_dir['basedir'] . '/hashcore-widgets/' . $name,
					$css
				);

			} else {
				wp_cache_add( $name, $css, 'hashcore_widgets' );
			}
	...
	public static function clear_file_cache( $force_delete = false ){
		...
		if( !get_transient('sow:cleared') || $force_delete ) {
			...
			if( WP_Filesystem() ) {
				...
				$list = $wp_filesystem->dirlist( $upload_dir['basedir'] . '/hashcore-widgets/' );
				if ( ! empty( $list ) ) {
					foreach($list as $file) {
						if( $file['lastmodunix'] < time() - self::$css_expire || $force_delete ) {
							// Delete the file
							$wp_filesystem->delete( $upload_dir['basedir'] . '/hashcore-widgets/' . $file['name'] );
						}
					}
				}
			}
	...
	public function get_instance_css( $instance ){
		...
		if( !class_exists('HashCore_Widgets_Less_Functions') ) require plugin_dir_path( __FILE__ ).'inc/less-functions.php';

		$style_name = $this->get_style_name($instance);
		if( empty($style_name) ) return '';

		$less_file = hashcore_widget_get_plugin_dir_path( $this->id_base ).'styles/'.$style_name . '.less';
		$less_file = apply_filters( 'hashcore_widgets_less_file_' . $this->id_base, $less_file, $instance, $this );
		...
		$less = apply_filters( 'hashcore_widgets_styles', $less, get_class($this), $instance );
		$less = apply_filters( 'hashcore_widgets_less_' . $this->id_base, $less, $instance, $this );
		...
		$compiler = new lessc();
		$lc_functions = new HashCore_Widgets_Less_Functions($this, $instance);
		...
		return apply_filters( 'hashcore_widgets_instance_css', $css, $instance, $this );
	}
	...
	private function get_less_import_contents($matches) {
		...
		$search_path = array(
			hashcore_widget_get_plugin_dir_path( $this->id_base ) . 'styles/',
	...
	function less_import_google_font($instance, $args) {
		...
		foreach ( $fonts as $font ) {
			$font_imports[] = hashcore_widget_get_font( $font );
		}
	...
	function enqueue_frontend_scripts( $instance ){
		...
		do_action('hashcore_widgets_enqueue_frontend_scripts_' . $this->id_base, $instance, $this);
	}
	...
	function is_preview( $instance = array() ){
		...
		return
			...
			!empty( $_GET['hashcore_panels_live_editor'] ) ||     // Is this a Page Builder live editor request
