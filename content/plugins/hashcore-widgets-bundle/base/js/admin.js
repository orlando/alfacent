...
(function($){
    ...
    $.fn.sowSetupForm = function() {
        ...
        return $(this).each( function(i, el){
            ...
            if( $el.is('.hashcore-widget-form-main') ) {
                ...
                $mainForm = $el.closest('.hashcore-widget-form-main');
            }
            formId = $mainForm.find('> .hashcore-widgets-form-id').val();

            // Find any field or sub widget fields.
            var $fields = $el.find('> .hashcore-widget-field');

            // Process any sub sections
            $fields.find('> .hashcore-widget-section').sowSetupForm();

            // Process any sub widgets whose fields aren't contained in a section
            $fields.filter('.hashcore-widget-field-type-widget:not(:has(> .hashcore-widget-section))').sowSetupForm();

            // Store the field names
            $fields.find('.hashcore-widget-input').each(function(i, input){
            ...
            $fields.find('> .hashcore-widget-field-repeater').sowSetupRepeater();

            // For any repeater items currently in existence
            $el.find('.hashcore-widget-field-repeater-item').sowSetupRepeaterItems();

            // Set up any color fields
            $fields.find('> .hashcore-widget-input-color').wpColorPicker();
            ...
            $fields.find('> .media-field-wrapper').each(function(){
                var $media = $(this);
                var $field = $media.closest('.hashcore-widget-field');
                ...
                $media.find('a.media-upload-button' ).click(function(e){
                    ...
                    var $$ = $(this);
                    var $c = $(this ).closest('.hashcore-widget-field');
            ...
            $fields.filter('.hashcore-widget-field-type-widget, .hashcore-widget-field-type-section').find('> label').click(function(){
                var $$ = $(this);
                $(this).toggleClass( 'hashcore-widget-section-visible' );
                $(this).siblings('.hashcore-widget-section').slideToggle(function(){
                    $(window).resize();
                    $(this).find('> .hashcore-widget-field-container-state').val($(this).is(':visible') ? 'open' : 'closed');
                });
            });
            ...
            var iconWidgetCache = {};
            $fields.filter('.hashcore-widget-field-type-icon').each(function(){
                var $$ = $(this),
                    $is = $$.find('.hashcore-widget-icon-selector'),
                    $v = $is.find('.hashcore-widget-icon-icon'),
                    $b = $$.find('.hashcore-widget-icon-selector-current');
                ...
                var rerenderIcons = function(){
                    var family = $is.find('select.hashcore-widget-icon-family').val();
                    var container = $is.find('.hashcore-widget-icon-icons');
                    ...
                    if( $('#'+'hashcore-widget-font-'+family).length === 0) {

                        $("<link rel='stylesheet' type='text/css'>")
                            .attr('id', 'hashcore-widget-font-' + family)
                    ...
                    for ( var i in iconWidgetCache[family].icons ) {
                        ...
                        var icon = $('<div data-sow-icon="' + iconWidgetCache[family].icons[i] +  '"/>')
                            ...
                            .addClass( 'hashcore-widget-icon-icons-icon' )
                            .click(function(){
                                var $$ = $(this);
                                if( $$.hasClass('hashcore-widget-active') ) {
                                    // This is being unselected
                                    $$.removeClass('hashcore-widget-active');
                                    $v.val( '' );
                                    ...
                                    container.find('.hashcore-widget-icon-icons-icon').removeClass('hashcore-widget-active');
                                    $$.addClass('hashcore-widget-active');
                                    $v.val( $$.data('value') );
                        ...
                        if( $v.val() === family + '-' + i ) {
                            if( !icon.hasClass('hashcore-widget-active') ) {
                                // This is becoming active, so simulate a click
                                icon.click();
                            }
                            icon.addClass('hashcore-widget-active');
                        }
                    }

                    // Move a selcted item to the first position
                    container.prepend( container.find('.hashcore-widget-active') );
                };
                ...
                var changeIconFamily = function(){
                    ...
                    if(typeof iconWidgetCache[family] === 'undefined') {
                        $.getJSON(
                            soWidgets.ajaxurl,
                            { 'action' : 'hashcore_widgets_get_icons', 'family' :  $is.find('select.hashcore-widget-icon-family').val() },
                            function(data) {
                ...
                $is.find('select.hashcore-widget-icon-family').change(function(){
                    $is.find('.hashcore-widget-icon-icons').empty();
                    changeIconFamily();
                });
            });
            ...
            $fields.filter('.hashcore-widget-field-type-slider').each(function(){
                var $$ = $(this);
                var $input = $$.find('input[type="number"]');
                var $c = $$.find('.hashcore-widget-value-slider');
                ...
                $c.slider({
                    ...
                    slide: function( event, ui ) {
                        ...
                        $$.find('.hashcore-widget-slider-value').html( ui.value );
                    }
                });
            });
            ...
            $fields.filter('.hashcore-widget-field-type-link').each( function(){
                var $$ = $(this);
                ...
                $$.on( 'click', '.posts li', function(e){
                    e.preventDefault();
                    var $li = $(this);
                    $$.find('input.hashcore-widget-input').val( 'post: ' + $li.data('ID') );
            ...
            $fields.filter('[data-state-emitter]').each( function(){
                ...
                $(this).find('.hashcore-widget-input').on('keyup change', stateEmitterChangeHandler);

                // Trigger initial state emitter changes
                $(this).find('.hashcore-widget-input').each(function(){
                    var $$ = $(this);
                    if( $$.is(':radio') ) {
            ...
            $el.find('.hashcore-widget-field-repeater-item').trigger('updateFieldPositions');
    ...
    $.fn.sowSetupPreview = function(){
        var $el = $(this);
        var previewButton = $el.siblings('.hashcore-widget-preview');
    ...
    $.fn.sowSetupRepeater = function(){
        ...
        return $(this).each( function(i, el){
            var $el = $(el);
            var $items = $el.find('.hashcore-widget-field-repeater-items');
            var name = $el.data('repeater-name');

            $items.bind('updateFieldPositions', function(){
                var $$ = $(this);
                var $rptrItems = $$.find('> .hashcore-widget-field-repeater-item');
                // Set the position for the repeater items
                $rptrItems.each(function(i, el){
                    $(el).find('.hashcore-widget-input').each(function(j, input){
                ...
                $$.find('.hashcore-widget-input').each(function(i, input){
                    var pos = $(input).data('repeater-positions');
                    var $in = $(input);
            ...
            $items.sortable( {
                handle : '.hashcore-widget-field-repeater-item-top',
                items : '> .hashcore-widget-field-repeater-item',
                update: function(){
            ...
            $el.find('> .hashcore-widget-field-repeater-add').disableSelection().click( function(e){
                e.preventDefault();
                $el.closest('.hashcore-widget-field-repeater')
                    .sowAddRepeaterItem()
                    .find('> .hashcore-widget-field-repeater-items').slideDown('fast', function(){
                        $(window).resize();
                    });
            } );

            $el.find('> .hashcore-widget-field-repeater-top > .hashcore-widget-field-repeater-expand').click( function(e){
                e.preventDefault();
                $el.closest('.hashcore-widget-field-repeater').find('> .hashcore-widget-field-repeateritems-').slideToggle('fast', function() {
					$(window).resize();
				});
            } );
        } );
    ...
    $.fn.sowAddRepeaterItem = function(){
        return $(this).each( function(i, el){
            ...
            var $el = $(el);
            var $nextIndex = $el.find('> .hashcore-widget-field-repeater-items').children().length+1;

            // Create an object with the repeater html so we can make some changes to it.
            var repeaterObject = $( '<div>' + $el.find('> .hashcore-widget-field-repeater-item-html').html() + '</div>' );
            repeaterObject.find('[data-name]').each( function(){
                var $$ = $(this);
                // Skip out items that are themselves inside repeater HTML wrappers
                if( $$.closest('.hashcore-widget-field-repeater-item-html').length === 0 ) {
            ...
            var item = $('<div class="hashcore-widget-field-repeater-item ui-draggable" />')
                .append(
                    $('<div class="hashcore-widget-field-repeater-item-top" />')
                        .append(
                            $('<div class="hashcore-widget-field-expand" />')
                        )
                        .append(
                            readonly ? '' : $('<div class="hashcore-widget-field-copy" />')
                        )
                        .append(
                            readonly ? '' : $('<div class="hashcore-widget-field-remove" />')
                        )
                        .append( $('<h4 />').html( $el.data('item-name') ) )
                )
                .append(
                    $('<div class="hashcore-widget-field-repeater-item-form" />')
                        .html( repeaterHtml )
                );

            // Add the item and refresh
            $el.find('> .hashcore-widget-field-repeater-items').append(item).sortable( "refresh").trigger('updateFieldPositions');
    ...
    $.fn.sowRemoveRepeaterItem = function () {
        return $(this).each( function(i, el){
            var $itemsContainer = $(this).closest('.hashcore-widget-field-repeater-items');
            $(this).remove();
    ...
    $.fn.sowSetupRepeaterItems = function () {
        return $(this).each(function (i, el) {
            ...
            if (typeof $el.data('sowrepeater-actions-setup') === 'undefined') {
                var $parentRepeater = $el.closest('.hashcore-widget-field-repeater');
                var itemTop = $el.find('> .hashcore-widget-field-repeater-item-top');
                ...
                itemTop.click(function (e) {
                    if (e.target.className === "hashcore-widget-field-remove" || e.target.className === "hashcore-widget-field-copy") {
                        return;
                    }
                    e.preventDefault();
                    $(this).closest('.hashcore-widget-field-repeater-item').find('.hashcore-widget-field-repeater-item-form').eq(0).slideToggle('fast', function () {
						$(window).resize();
                        if($(this).is(':visible')) {
                ...
                itemTop.find('.hashcore-widget-field-remove').click(function (e) {
                    e.preventDefault();
                    if ( confirm( soWidgets.sure ) ) {
                        var $s = $(this).closest('.hashcore-widget-field-repeater-items');
                        $(this).closest('.hashcore-widget-field-repeater-item').slideUp('fast', function () {
                            $(this).remove();
                ...
                itemTop.find('.hashcore-widget-field-copy').click(function(e){
                    e.preventDefault();
                    var $form = $(this).closest('.hashcore-widget-form-main');
                    var $item = $(this).closest('.hashcore-widget-field-repeater-item');
                    var $copyItem = $item.clone();
                    var $items = $item.closest('.hashcore-widget-field-repeater-items');
                    ...
                    $copyItem.find( '*[name]' ).each( function () {
                        ...
                        // Color field :/
                            ...
                            var $soWidgetField = $inputElement.closest('.hashcore-widget-field');
                        ...
                        if(id) {
                            ...
                            if (!newIds[idBase]) {
                                newIds[idBase] = $form.find('.hashcore-widget-input[id^=' + idBase + ']').not('[id*=_id_]').length + 1;
                            }
                        ...
                        var nestLevel = $item.parents('.hashcore-widget-field-repeater').length;
                        var $body = $('body');
                ...
                $el.find('> .hashcore-widget-field-repeater-item-form').sowSetupForm();
    ...
    var sowForms = {
        ...
        getRepeaterId: function( $el ) {
            ...
            var $r = $el.closest('.hashcore-widget-field-repeater-item');
            if( $r.length ) {
    ...
    $('.widgets-holder-wrap').on('click', '.widget:has(.hashcore-widget-form-main) .widget-top', function(){
        var $$ = $(this).closest('.widget').find('.hashcore-widget-form-main');
        setTimeout( function(){
            $$.sowSetupForm();
        }, 200);
    });
    ...
    if( $('body').hasClass('wp-customizer') ) {
        ...
        $(document).on('widget-added', function (e, widget) {
            widget.find('.hashcore-widget-form').sowSetupForm();
        });
    }

    // When we open a Page Builder widget dialog
    $(document).on('dialogopen', function(e){
        $(e.target).find('.hashcore-widget-form-main').sowSetupForm();
    });
